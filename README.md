...
현재 `v14.2.0`. 버전 앞자리는 **기수**를 뜻합니다(2026-09-12부터 `1.x.x` →
`14.x.x`로 전환 — 기존 `v1.0.0`/`v1.1.0`/`v1.2.0` 태그와 GitHub Release는
같은 커밋을 가리키는 `v14.0.0`/`v14.1.0`/`v14.2.0`로 옮겼습니다). 다음
기수가 버전을 새로 시작하려면 `15.0.0`부터로 하세요. `pre-supabase-migration`
태그는 예전에 Supabase 도입을 검토하다 접었을 때 남긴 지점입니다.
